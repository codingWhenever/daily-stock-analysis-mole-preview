...
- [新功能] 新增公募基金池 MVP，支持基金入池、公开净值缓存、风险收益指标、同类分位和规则化申购/定投/观望/赎回信号，不接入个人账户或持仓导入。
- [改进] 基金池指标改为 direct-first：阶段收益优先使用公开同类排行字段，最大回撤/年化波动/夏普优先使用平台风险指标，覆盖缺失时再回退本地净值计算。
- [新功能] Web 新增“选基”搜索体验，支持按基金名称、代码或拼音查询公募基金，直出公开榜单净值、日涨幅、多周期收益、同类排名/分位和费率，并可加入基金池生成基金类建议。
- [改进] 选基新增基金画像与数据覆盖底座，结构化记录资产类别、策略族、风格标签、市场上下文缺口、数据来源覆盖和后续回测/策略校准准备度。
- [改进] 选基的多周期收益展示从卡片网格改为核心阶段摘要、重点周期对照和连续阶段条带，保留同类百分位但降低罗列感。
- [改进] 选基的信号诊断合并为“触发依据 + 数据可信度”模块，并将数据覆盖的后续层升级为能力路线图，明确下一步接入分类型参数、市场上下文、回测校准和 LLM 审阅。
- [改进] 选基基金画像升级到 v2，新增分类型初始策略参数、市场上下文代理、回测校准准备度和信号上下文，明确当前参数为未回测初始规则。
- [改进] 选基数据底座继续补齐公开指数估值/风格轮动、基金行业配置/重仓股/定期报告、交易规则/申赎费率和资讯佐证结构，并在策略底座中以投研卡片展示。
- [新功能] 选基新增基金 NAV 滚动回测，基于本地缓存公开净值逐期生成历史信号，对比买入持有并展示策略收益、超额收益、最大回撤、命中率、交易次数和费用假设。
- [改进] 选基同类比较改为分位色阶视图，按领先、中位、偏弱和落后用不同颜色展示整体位置、排名样本、多周期同类分位和强弱周期。
- [修复] 修复选基搜索结果卡片操作按钮在两列布局下断行的问题，并为空收益榜单搜索结果增加兜底，避免宽泛关键词搜索因 returns 为空返回 400。
- [新功能] 选基新增基金账本/分类能力，支持创建不同主题色账本、顶部大分类切换、搜索加入当前账本，以及在基金卡片上手动调整账本归属。
- [文档] 新增选基决策系统数据基座、持仓导入与荐基规格，明确三平台截图导入、金额闭眼模式、用户画像、市场热点、荐基榜、回测校准和强决策动作规划。
- [文档] 新增选基产品目标、当前状态与路线评审稿，汇总已实现边界、未实现能力和 P0-P7 后续优先级。
- [文档] 补充选基评审结论，明确市场级买入/卖出榜仅汇总公开客观交易与资金流数据，个人持仓动作建议需叠加用户画像和已确认持仓，并同步直接 OCR 导入、轻量用户画像、`/funds` 金额闭眼和工作台 UI/UX 改版路线。
- [文档] 更新选基执行计划的下一轮路线修订，统一市场级榜单、个人持仓动作和 `/funds` 工作台 UI/UX 并行推进口径。
- [新功能] 选基新增市场级公开榜单 API，汇总 ETF 资金流入/流出、ETF 成交热度和开放式基金收益实证，并显式标记公开数据不支持真实买卖金额、笔数或个性化动作。
- [改进] 选基页面重构为决策工作台，市场榜单、搜索入口和个人基金池分区展示，基金池改为可滚动列表 + 当前选中基金详情，避免已添加产品过多时页面无限拉长。
- [新功能] 选基新增基金持仓导入后端底座，支持截图/OCR 文本预览、用户确认后写入平台账本持仓当前快照，并保持同基金跨平台明细分开、全部视图按代码聚合。
- [新功能] Web 选基新增持仓导入助手入口，支持平台选择、截图/文本预览、候选行编辑、金额默认闭眼和确认覆盖写入。
- [新功能] 选基新增市场级荐基证据卡 API，基于公开榜单候选叠加本地分析/回测准备度、风险边界和失效条件，不生成个人买卖动作。
- [改进] Web 选基接入今日市场级荐基证据卡，在公开榜单右侧展示候选证据、回测准备度、风险边界和失效条件，并保留加入基金池/刷新分析动作。
- [新功能] Web 选基新增账本画像问卷 v1，可在单个账本下维护账户类型、风险目标、投资期限、调仓频率、资金用途和备注，作为后续个性化决策输入。
- [改进] Web 选基持仓导入助手新增已确认持仓快照视图，展示按基金代码聚合的全部口径和各平台账本明细，并沿用金额闭眼模式。
- [新功能] 选基新增个人持仓动作建议 API，基于已确认持仓、账本画像和单品分析输出加仓/定投/持有/减仓/卖出观察或阻塞项。
- [新功能] Web 选基新增个人持仓动作面板，展示个人动作数量、阻塞项、置信度和失效条件，并在导入持仓、保存画像或刷新分析后自动更新。
...
- [改进] Web 选基工作台新增 sticky 分区快捷导航（市场榜单/搜基/持仓/个人动作/账本/基金池），支持点击锚点跳转和滚动高亮，缓解长页面纵向滚动成本；移动端避让顶部悬浮工具条且分区标签可横向滚动。
- [文档] 新增选基模块产品 + UI/UX 走查与工程改造单 `docs/funds-uiux-review-2026-06-26.md`，给出可直接排期的分级问题清单、目标信息架构与改造任务表。
- [改进] 选基荐基证据卡默认精简，仅展示正向市场证据，将回测准备/单品画像/风险标记/失效条件收进“查看证据与边界”折叠，降低负面信息堆叠感。
- [改进] 选基个人持仓动作在无可执行动作时降级为单条引导态（阻塞原因 + 下一步指引），逐条阻塞明细收进“查看 N 条阻塞明细”折叠，避免空值卡片平铺撑高页面。
- [改进] 选基荐基与个人动作区新增“评分说明”悬浮图例，解释 Score/总分口径与子项含义；个人动作金额闭眼时持仓卡改为紧凑展示净值与浮盈浮亏方向，不再整列遮蔽。
- [改进] 选基页头新增“数据状态”统一口径图例（已实接/计算中·代理/待接入/需用户确认），明确缺口不会显示为已接入；“决策分层”由整块横幅降级为紧凑提示，避免重复占位。
- [改进] 选基基金池统计卡（产品数/可执行信号/数据可用）从页面中部上移并入基金池分区头部，减少与账本计数语义重复及空值大卡。
...
- [改进] Web 选基账本画像区支持直接编辑账本名称和主题色，保存时复用现有账本更新接口并保留重名校验。
