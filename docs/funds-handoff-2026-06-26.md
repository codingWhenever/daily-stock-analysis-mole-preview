...
当前 `/funds` 已从“基金搜索和基金池展示”推进到“公开数据可解释、数据质量可追踪、费用/类型/回测有结构化底座、基金账本具备后端轻量账户画像”的阶段。它还不是完整投资决策系统：资讯佐证实接、账本交互增强、真实持仓导入、用户画像、荐基榜、组合级账户收益和回撤仍未完成。

接手时要把当前能力理解为“公开基金数据 + 本地基金池/账本 + 规则化研究信号”的阶段，而不是“已经能结合个人真实持仓和用户画像做完整买卖决策”的阶段。`docs/funds-decision-foundation-spec.md` 中的持仓导入、金额闭眼模式、用户画像、荐基榜、市场级买入/卖出/资金流榜单、个人持仓动作建议和 `/funds` 工作台 UI/UX 改版都是下一阶段蓝图，不能写入当前已完成清单。
...
- 后续支持真实持仓、账本/账户、用户画像、市场热点、行业 Top 产品、市场级买入/卖出/资金流榜单、个人持仓动作建议和闭环反馈。

当前实现只覆盖公开数据和本地基金池/账本，不接个人平台账号，不读取真实持仓成本，不承诺收益。

和完整规格的关系：

- 已实现主线：公开基金搜索、基金池、数据质量、类型画像、费用模型、NAV 回测、回测校准、规则信号、轻量账本画像。
- 仍是规划主线：真实持仓导入、用户画像问卷、金额闭眼模式、组合级持仓分析、荐基榜、热点行业 Top10、行业下产品 Top10、市场级买入/卖出/资金流榜单、个人持仓动作建议、页面工作台 UI/UX 改版。
...
本轮没有提交 git commit。工作树中 `/funds` 相关能力仍是未提交改动和新增文件，接手者不要误删或回滚。
...
- `src/services/fund_backtest_calibration.py`
- `tests/test_fund_service.py`
- `apps/dsa-web/src/api/funds.ts`
- `apps/dsa-web/src/pages/FundsPage.tsx`
...
`docs/funds-decision-foundation-spec.md` 规划了更完整的基金决策系统。以下内容目前仍未落地，不能在产品中宣称已支持：
...
- 用户画像问卷。
- 根据风险等级、回撤预算、投资期限、流动性和偏好限制生成个人持仓动作建议。
...
- 按市场阶段和基金类型自动校准策略阈值。
- 用户反馈与后验评估闭环。
- `/funds` 工作台 UI/UX 改版为“今日决策 / 我的持仓 / 荐基 / 基金池 / 导入与数据 / 用户画像”的信息架构，并按功能契约分步落地。
...
- 不要把个人平台账本/真实持仓说成已接入。
- 不要把完整规格里的荐基榜、金额闭眼模式、用户画像和持仓导入 API 草案当作当前已存在 API。
